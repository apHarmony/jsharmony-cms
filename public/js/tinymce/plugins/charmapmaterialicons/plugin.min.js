...
(function (domGlobals) {
  ...
  var closestClass = function(el, className){
    if(!el) return null;
    do {
      var classList = el.classList;
      if(classList){
        for(var i=0;i<classList.length;i++){
          if(classList[i]==className) return el;
        }
      }
  ...
  var insertChar = function (editor, chr) {
    var evtChr;
    if(chr.substr(0,13)=='materialicon_'){
      //Check if material-icons class is already on a parent
      if(!closestClass(editor.selection.getNode(), 'material-icons')){
        evtChr = '<span class="material-icons">'+chr.substr(13)+'</span>';
      }
      else {
        evtChr = chr.substr(13);
      }
    }
    else {
      evtChr = fireInsertCustomChar(editor, chr).chr;
    }
  ...
  var getCharMapAppendTitle = function (editor) {
    return editor.settings.charmap_append_title || 'User Defined';
  };
  ...
  var extendCharMap = function (editor, charmap) {
    var userDefined = getCharMapAppendTitle(editor);
